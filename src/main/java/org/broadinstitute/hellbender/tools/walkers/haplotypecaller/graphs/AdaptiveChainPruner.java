...
public class AdaptiveChainPruner<V extends BaseVertex, E extends BaseEdge> extends ChainPruner<V,E> {
    ...
    public AdaptiveChainPruner(final double initialErrorProbability, final double logOddsThreshold) {
    ...
    protected List<Path<V,E>> chainsToRemove(final List<Path<V, E>> chains) {
        ...
        List<Path<V,E>> probableErrorChains = likelyErrorChains(chains, graph, initialErrorProbability);
    ...
    private List<Path<V,E>> likelyErrorChains(final List<Path<V, E>> chains, final BaseGraph<V,E> graph, final double errorRate) {
        return chains.stream().filter(c -> chainLogOdds(c, graph, errorRate) < logOddsThreshold).collect(Collectors.toList());
    }
    ...
    private double chainLogOdds(final Path<V,E> chain, final BaseGraph<V,E> graph, final double errorRate) {
        ...
        final double leftLogOdds = leftTotalMultiplicity == leftMultiplicity ? 0.0 :
                Mutect2Engine.lnLikelihoodRatio(leftTotalMultiplicity, leftMultiplicity, errorRate);
        final double rightLogOdds = rightTotalMultiplicity == rightMultiplicity ? 0.0 :
                Mutect2Engine.lnLikelihoodRatio(rightTotalMultiplicity, rightMultiplicity, errorRate);
